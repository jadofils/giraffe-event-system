...
class OrganizationController {
    ...
    static create(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                const { organizationName, description, contactEmail, contactPhone, address, organizationType, city, country, postalCode, stateProvince, members, assignCreator = !isAdmin, // Default to true for regular users, false for admins
                 } = req.body;
                ...
                if (req.files && req.files["supportingDocument"]) {
                    const docFiles = req.files["supportingDocument"];
                    ...
                    for (const docFile of docFiles) {
                        if (!allowedTypes.includes(docFile.mimetype)) {
                            res.status(400).json({
                                success: false,
                                message: "Only PDF and image files are allowed as supporting documents.",
                            });
                            return;
                        }
                        try {
                            const uploadResult = yield CloudinaryUploadService_1.CloudinaryUploadService.uploadBuffer(docFile.buffer, "uploads/organization-supporting-document");
                ...
                if (req.files && req.files["logo"]) {
                    ...
                    try {
                        const uploadLogoResult = yield CloudinaryUploadService_1.CloudinaryUploadService.uploadBuffer(logoFile.buffer, "uploads/organization-logo");
                        ...
                        if (supportingDocuments.length > 0) {
                            try {
                                yield CloudinaryUploadService_1.CloudinaryUploadService.deleteFromCloudinary(supportingDocuments[0], uploadedFiles[0].type);
                            }
                            catch (cleanupError) {
                                console.error("Failed to cleanup supporting document:", cleanupError);
                            }
                        }
                        res.status(500).json({
                            success: false,
                ...
                const orgData = {
                    ...
                    logo: logoUrl,
                    status: isAdmin
                        ? OrganizationStatusEnum_1.OrganizationStatusEnum.APPROVED
                        : OrganizationStatusEnum_1.OrganizationStatusEnum.PENDING,
                };
                ...
                if (!result.success || !((_c = result.data) === null || _c === void 0 ? void 0 : _c.length)) {
                    ...
                    for (const file of uploadedFiles) {
                        try {
                            yield CloudinaryUploadService_1.CloudinaryUploadService.deleteFromCloudinary(file.url, file.type);
                        }
                        catch (cleanupError) {
                            console.error(`Failed to cleanup file ${file.url}:`, cleanupError);
                        }
                    }
                    res.status(400).json(Object.assign(Object.assign({}, result), { message: result.message || "Failed to create organization" }));
                    return;
                }
                ...
                if (assignCreator && userId) {
                    ...
                    const assignResult = yield OrganizationRepository_1.OrganizationRepository.assignUsersToOrganization([userId], result.data[0].organizationId);
                    if (!assignResult.success) {
                        ...
                        try {
                            yield OrganizationRepository_1.OrganizationRepository.delete(result.data[0].organizationId);
                        }
                        catch (deleteError) {
    ...
    static updateLogo(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                const orgResult = yield OrganizationRepository_1.OrganizationRepository.getById(id);
                ...
                try {
                    ...
                    const uploadResult = yield CloudinaryUploadService_1.CloudinaryUploadService.uploadBuffer(logoFile.buffer, "uploads/organization-logo");
                    ...
                    if (!updateResult.success) {
                        ...
                        try {
                            yield CloudinaryUploadService_1.CloudinaryUploadService.deleteFromCloudinary(uploadResult.url, "image");
                        }
                        catch (cleanupError) {
                            console.error("Failed to cleanup new logo after update failure:", cleanupError);
                        }
                    ...
                    if (oldLogoUrl) {
                        ...
                        try {
                            yield CloudinaryUploadService_1.CloudinaryUploadService.deleteFromCloudinary(oldLogoUrl, "image");
                    ...
                    const { CacheService } = yield Promise.resolve().then(() => __importStar(require("../services/CacheService")));
                    yield CacheService.invalidateMultiple(["org:all", `org:id:${id}`]);
     ...
     * @access Protected
     */
    static updateSupportingDocument(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            const { id } = req.params;
            if (!id || !OrganizationRepository_1.OrganizationRepository.UUID_REGEX.test(id)) {
                res
            ...
            try {
                ...
                for (const oldUrl of oldDocUrls) {
                    if (oldUrl) {
                        ...
                        try {
                            yield CloudinaryUploadService_1.CloudinaryUploadService.deleteFromCloudinary(oldUrl, resourceType);
                            console.log(`✅ Deleted old document: ${oldUrl}`);
                        }
                        catch (deleteErr) {
                            console.warn(`❌ Failed to delete old document ${oldUrl}:`, deleteErr);
                ...
                for (const file of files) {
                    try {
                        const uploadResult = yield CloudinaryUploadService_1.CloudinaryUploadService.uploadBuffer(file.buffer, "uploads/organization-supporting-document");
                        ...
                        for (const cleanupFile of uploadedFilesToCleanup) {
                            try {
                                yield CloudinaryUploadService_1.CloudinaryUploadService.deleteFromCloudinary(cleanupFile.url, cleanupFile.type);
                            }
                            catch (cleanupErr) {
                                console.error(`Failed to cleanup newly uploaded file ${cleanupFile.url}:`, cleanupErr);
                            }
                        }
                        res.status(500).json({
                            success: false,
                ...
                const result = yield OrganizationRepository_1.OrganizationRepository.update(id, {
                ...
                console.error(`[OrganizationController UpdateSupportingDocument Error] ID: ${id}:`, error);
    ...
    static delete(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                const orgCheck = yield OrganizationRepository_1.OrganizationRepository.getById(id);
                ...
                console.log("Organization found with:", {
                    venueCount: ((_b = organization.venues) === null || _b === void 0 ? void 0 : _b.length) || 0,
                    userCount: ((_c = organization.users) === null || _c === void 0 ? void 0 : _c.length) || 0,
                });
                ...
                if (organization.venues && organization.venues.length > 0) {
                    ...
                    const venueResult = yield OrganizationRepository_1.OrganizationRepository.removeVenuesFromOrganization(venueIds, id);
                ...
                if (organization.users && organization.users.length > 0) {
                    console.log("Removing users:", organization.users.map((u) => u.userId));
                    const userResult = yield OrganizationRepository_1.OrganizationRepository.removeUsersFromOrganization(organization.users.map((u) => u.userId), id);
                ...
                console.error(`[OrganizationController Delete Error] Org ID: ${id}:`, error);
                res.status(500).json({
                    success: false,
                    message: "Internal server error",
                    error: error instanceof Error ? error.message : "Unknown error",
    ...
    static assignUsers(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (userIds.some((uid) => !OrganizationRepository_1.OrganizationRepository.UUID_REGEX.test(uid))) {
                res.status(400).json({
                    success: false,
            ...
            try {
                ...
                if (orgCheck.data.status !== OrganizationStatusEnum_1.OrganizationStatusEnum.APPROVED) {
                    res.status(400).json({
                        success: false,
                ...
                if (result.success) {
                    res.status(200).json({
                        success: true,
                        data: result.data,
                        message: result.message || "Users successfully assigned to organization",
                    });
                }
                else {
                    // Handle different failure scenarios
                    if ((_b = result.message) === null || _b === void 0 ? void 0 : _b.includes("already assigned")) {
                        res.status(409).json({
                            success: false,
                            message: result.message,
                        });
                    }
                    else if ((_c = result.message) === null || _c === void 0 ? void 0 : _c.includes("not found")) {
                        res.status(404).json({
                            success: false,
                            message: result.message,
                        });
    ...
    static removeUsers(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                const orgUsers = yield OrganizationRepository_1.OrganizationRepository.getUsersByOrganization(id);
                if (orgUsers.success && orgUsers.data) {
                    const adminUsers = orgUsers.data.filter((user) => user.role.roleName === "ADMIN");
                    if (adminUsers.length > 0) {
                        const remainingAdmins = adminUsers.filter((admin) => !userIds.includes(admin.userId));
                ...
                if (result.success) {
                    res.status(200).json({
                        success: true,
                        data: result.data,
                        message: result.message || "Users successfully removed from organization",
                    });
                }
                else {
                    // Handle different failure scenarios
                    if ((_b = result.message) === null || _b === void 0 ? void 0 : _b.includes("not found")) {
                        res.status(404).json({
                            success: false,
                            message: result.message,
                        });
                    }
                    else if ((_c = result.message) === null || _c === void 0 ? void 0 : _c.includes("Cannot remove")) {
                        res.status(400).json({
                            success: false,
                            message: result.message,
                        });
                        ...
                        res.status(400).json({
                            success: false,
                            message: result.message || "Failed to remove users from organization",
                        });
                    }
                }
            }
    ...
    static addVenues(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!organizationId ||
                !OrganizationRepository_1.OrganizationRepository.UUID_REGEX.test(organizationId)) {
                res.status(400).json({
                    success: false,
            ...
            if (!Array.isArray(venueIds) ||
                venueIds.length === 0 ||
                venueIds.some((vid) => !OrganizationRepository_1.OrganizationRepository.UUID_REGEX.test(vid))) {
                res.status(400).json({
                    success: false,
    ...
    static removeVenues(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                const result = yield OrganizationRepository_1.OrganizationRepository.removeVenuesFromOrganization(venueIds, organizationId);
                if (result.success) {
    ...
    static enableStatus(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                const result = yield OrganizationRepository_1.OrganizationRepository.enableOrganization(id);
                ...
                console.error(`[OrganizationController EnableStatus Error] ID: ${id}:`, error);
                res.status(500).json({
                    success: false,
                    message: "Internal server error",
                    error: error instanceof Error ? error.message : "Unknown error",
    ...
    static disableStatus(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                const result = yield OrganizationRepository_1.OrganizationRepository.disableOrganization(id);
                ...
                console.error(`[OrganizationController DisableStatus Error] ID: ${id}:`, error);
                res.status(500).json({
                    success: false,
                    message: "Internal server error",
                    error: error instanceof Error ? error.message : "Unknown error",
    ...
    static getOrganizationVenues(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                console.log("User Role:", isAdmin ? "ADMIN" : "REGULAR USER");
                const result = yield OrganizationRepository_1.OrganizationRepository.getById(organizationId);
                ...
                if (!isAdmin && result.data.status === OrganizationStatusEnum_1.OrganizationStatusEnum.DISABLED) {
                    res.status(403).json({
                        success: false,
                ...
                res.status(200).json({
                    ...
                    message: ((_b = result.data.venues) === null || _b === void 0 ? void 0 : _b.length) > 0
     ...
     * @access Protected
     */
    static getApprovedVenuesByOrganizationId(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            const { organizationId } = req.params;
            if (!organizationId ||
                !OrganizationRepository_1.OrganizationRepository.UUID_REGEX.test(organizationId)) {
            ...
            try {
                const orgResult = yield OrganizationRepository_1.OrganizationRepository.getById(organizationId);
                if (!orgResult.success || !orgResult.data) {
                    res
                        .status(404)
                        .json({ success: false, message: "Organization not found" });
                ...
                const approvedVenues = (orgResult.data.venues || []).filter((venue) => venue.status === "APPROVED");
                res.status(200).json({
                    success: true,
                    data: approvedVenues,
                    message: approvedVenues.length > 0
    ...
    static getOrganizationsByUserId(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            const { userId } = req.params;
            if (!userId || !OrganizationRepository_1.OrganizationRepository.UUID_REGEX.test(userId)) {
                res
                    .status(400)
            ...
            try {
                ...
                console.error(`[OrganizationController GetOrganizationsByUserId Error] User ID: ${userId}:`, error);
                res.status(500).json({
                    success: false,
                    message: "Internal server error",
                    error: error instanceof Error ? error.message : "Unknown error",
     ...
     * @access Public
     */
    static getPublicDetails(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            const { id } = req.params;
            if (!id || !OrganizationRepository_1.OrganizationRepository.UUID_REGEX.test(id)) {
                res.status(400).json({
            ...
            try {
                const result = yield OrganizationRepository_1.OrganizationRepository.getPublicDetails(id);
                if (!result.success || !result.data) {
                    res.status(404).json({
                        success: false,
                ...
                const { organizationId, organizationName, description, logo, address, city, country, members, venues, events, } = result.data;
                res.status(200).json({
                    success: true,
                    data: {
                        organizationId,
                        ...
                        country,
                        members,
                        venues: (venues === null || venues === void 0 ? void 0 : venues.map((venue) => ({
                ...
                console.error(`[OrganizationController GetPublicDetails Error] ID: ${id}:`, error);
                res.status(500).json({
                    success: false,
                    message: "Internal server error",
                    error: error instanceof Error ? error.message : "Unknown error",
     ...
     * @access Public
     */
    static getAllPublicOrganizations(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const result = yield OrganizationRepository_1.OrganizationRepository.getAllPublicOrganizations();
                if (!result.success) {
                    res.status(500).json({
                        success: false,
     ...
     * @access Admin Only
     */
    static queryOrganization(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            var _a;
            const { id } = req.params;
            const { reason } = req.body;
            ...
            const result = yield OrganizationRepository_1.OrganizationRepository.queryOrganization(id, reason);
     ...
     * @access Protected
     */
    static requestOrganizationAgain(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            const { id } = req.params;
            const result = yield OrganizationRepository_1.OrganizationRepository.requestOrganizationAgain(id);
            res.status(result.success ? 200 : 400).json(result);
        });
    }
}
