        ...
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                if (!Array.isArray(rolesData)) {
                    res.status(400).json({
                        success: false,
                        message: "Expected an array of roles",
                    });
                    return;
                }
                const results = [];
                for (const data of rolesData) {
                    ...
                    const roleOrError = yield RoleRepository_1.RoleRepository.createRole({
                        roleName,
                        description,
                        permissions,
                    });
                    if ("error" in roleOrError) {
                        results.push({
                            success: false,
                            message: roleOrError.error,
                            roleName,
                        });
                        continue;
                    }
                    const result = yield RoleRepository_1.RoleRepository.saveRole(roleOrError);
                    results.push(result);
                }
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                if (roles.length === 0) {
                    res.status(404).json({
                        success: false,
                        message: `No roles found matching '${roleName}'.`,
                    });
                    return;
                }
                ...
                res.status(500).json({
                    success: false,
                    message: "Failed to retrieve roles due to a server error.",
                });
            }
        });
    }
