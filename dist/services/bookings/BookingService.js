...
function checkConflict(bookingData) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            const existingBooking = yield bookingRepo.createQueryBuilder("booking")
                ...
                .andWhere("booking.startTime = :startTime AND booking.endTime = :endTime", { startTime: bookingData.startTime, endTime: bookingData.endTime })
                .getOne();
            if (existingBooking) {
            ...
            const conflictingBookings = yield bookingRepo.createQueryBuilder("booking")
                .where("booking.venueVenueId = :venueId", { venueId: bookingData.venueId })
                .andWhere("(booking.startDate <= :endDate AND booking.endDate >= :startDate)", { startDate: startDate.toISOString().split('T')[0],
            ...
            for (const booking of conflictingBookings) {
                ...
                const existingStartTotalMinutes = parseInt(existingStartTimeParts[0]) * 60 + parseInt(existingStartTimeParts[1]);
                const existingEndTotalMinutes = parseInt(existingEndTimeParts[0]) * 60 + parseInt(existingEndTimeParts[1]);
