...
export class VenueController {
  ...
  static async createMultiple(req: AuthenticatedRequest, res: Response): Promise<void> {
    const userId = req.user?.userId;
  ...
  static async updateVenueManager(req: AuthenticatedRequest, res: Response): Promise<void> {
    ...
    try {
      const result = await VenueRepository.updateVenueManager(venueId, managerId);
      if (result.success && result.data) {
  ...
  static async searchVenues(req: Request, res: Response): Promise<void> {
    ...
    if (minCapacity && !isNaN(Number(minCapacity))) criteria.minCapacity = Number(minCapacity);
    if (maxCapacity && !isNaN(Number(maxCapacity))) criteria.maxCapacity = Number(maxCapacity);
