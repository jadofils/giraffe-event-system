...
export class VenueBookingController {
  ...
  static async createBooking(req: Request, res: Response): Promise<void> {
    try {
      ...
      const errors = await validate(bookingInstance, { forbidUnknownValues: true });
      if (errors.length > 0) {
        res.status(400).json({
          success: false,
      ...
      // Create booking
      const result = await VenueBookingRepository.createBooking(bookingInstance);
      if (!result.success) {
  ...
  // Create multiple bookings
   static async createMultipleBookings(req: Request, res: Response): Promise<void> {
    try {
      ...
      const invalidVenue = venues.find(
        v => v.organizationId !== firstOrgId || v.location !== firstLocation
      );
      if (invalidVenue) {
        res.status(400).json({
          success: false,
      ...
      const result = await VenueBookingRepository.createMultipleBookings(bookingsData);
  ...
  // Update a booking
   static async updateBooking(req: Request, res: Response): Promise<void> {
    try {
      ...
      const result = await VenueBookingRepository.updateBooking(id, bookingData);
  ...
  // Update booking status
   static async updateBookingStatus(req: Request, res: Response): Promise<void> {
    try {
      ...
      const result = await VenueBookingRepository.updateBookingStatus(id, normalizedStatus);
      if (!result.success) {
  ...
  // Get bookings by status
   static async getBookingsByStatus(req: Request, res: Response): Promise<void> {
    try {
      ...
      const result = await VenueBookingRepository.getBookingsByStatus(status as ApprovalStatus);
      res.status(result.success ? 200 : 404).json({
        success: result.success,
        message: result.message,
        data: result.data,
  ...
  // Get bookings by date range
   static async getBookingsByDateRange(req: Request, res: Response): Promise<void> {
    try {
      ...
      const filterOptions = (req.query.filterOptions as string)?.split(',') as ('min' | 'hours' | 'days' | 'all')[] || ['all'];
      ...
      const result = await VenueBookingRepository.getBookingsByDateRange(start, end, filterOptions);
      res.status(result.success ? 200 : 404).json({
        success: result.success,
        message: result.message,
        data: result.data,
  ...
  // Get total booking amount for an event
   static async getTotalBookingAmountForEvent(req: Request, res: Response): Promise<void> {
    try {
      const { eventId } = req.params;
      const result = await VenueBookingRepository.getTotalBookingAmountForEvent(eventId);
      res.status(result.success ? 200 : 404).json({
        success: result.success,
        message: result.message,
