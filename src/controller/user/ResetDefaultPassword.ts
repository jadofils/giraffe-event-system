...
export class ResetPasswordController {
  ...
  static async resetPassword(req: Request, res: Response): Promise<void> {
    ...
    if (!token || !password || !confirm_password) {
      console.log(
        "[Password Reset Attempt] Missing token, password, or confirm_password"
      );
      res
        .status(400)
        .json({
    ...
    if (isTokenUsed) {
      console.log(
        `[Password Reset Attempt] Token: ${token} - Token already used`
      );
      res
        .status(400)
        .json({
    ...
    try {
      ...
      console.log(
        `[Password Reset Attempt] Decoded token: ${JSON.stringify(decoded)}`
      );

      if (decoded.purpose !== "password_reset") {
        console.log(
          `[Password Reset Attempt] User ID: ${decoded.userId} - Invalid token purpose`
        );
        res
          .status(403)
          .json({
      ...
      if (!ResetPasswordController.UUID_REGEX.test(decoded.userId)) {
        console.log(
          `[Password Reset Attempt] User ID: ${decoded.userId} - Invalid UUID format`
        );
        res
          .status(400)
      ...
      if (!user) {
        console.log(
          `[Password Reset Attempt] User ID: ${decoded.userId} - User not found`
        );
      ...
      // Send success email
      try {
        await EmailService.sendSuccessPasswordForgetEmail(
          user.email,
          user.username
        );
        console.log(
          `[Password Reset Success] Success email sent to ${user.email}`
        );
      } catch (emailError) {
        console.error(
          `[Password Reset Success] Failed to send success email to ${user.email}:`,
          emailError
        );
      }

      console.log(
        `[Password Reset Success] User ID: ${user.userId}, Email: ${user.email}`
      );
      ...
      console.error(
        `[Password Reset Error] Token: ${token} - ${
  ...
  static async forgotPasswordLink(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      if (!user) {
        console.log(
          `[Password Reset Attempt] Email: ${email} - User not found`
        );
        res.status(200).json({
          success: true,
          message:
      ...
      if (!ResetPasswordController.UUID_REGEX.test(user.userId)) {
        console.log(
          `[Password Reset Attempt] User ID: ${user.userId} - Invalid UUID format`
        );
        res
          .status(500)
          .json({ success: false, message: "Invalid user ID format" });
      ...
      if (attempts >= RESET_EMAIL_RATE_LIMIT) {
        console.log(
          `[Password Reset Attempt] Email: ${email} - Rate limit exceeded`
        );
        res
          .status(429)
          .json({
      ...
      console.log(
        `[Password Reset Success] Email sent to: ${user.email}, Reset Link: ${resetLink}`
      );
      ...
      console.error(
        `[Password Reset Error] Email: ${email} - ${
          error instanceof Error ? error.message : "Unknown error"
        }`
      );
      res
   ...
   * @access Public
   */
  static async resendPasswordResetEmail(
    req: Request,
    res: Response
  ): Promise<void> {
    const { email } = req.body;
    ...
    if (!email || !token) {
      console.log(
        `[Password Reset Attempt] Email: ${email}, Token: ${token} - Missing email or token`
      );
      res
        .status(400)
    ...
    try {
      ...
      if (
        ...
        console.log(
          `[Password Reset Attempt] Email: ${email}, Token: ${token} - Invalid or mismatched token`
        );
        res
          .status(400)
      ...
      if (!user) {
        console.log(
          `[Password Reset Attempt] Email: ${email}, Token: ${token} - User not found`
        );
        res.status(200).json({
          success: true,
          message:
      ...
      if (!ResetPasswordController.UUID_REGEX.test(user.userId)) {
        console.log(
          `[Password Reset Attempt] Email: ${email}, Token: ${token}, User ID: ${user.userId} - Invalid UUID format`
        );
        res
          .status(500)
          .json({ success: false, message: "Invalid user ID format" });
      ...
      if (attempts >= RESET_EMAIL_RATE_LIMIT) {
        console.log(
          `[Password Reset Attempt] Email: ${email}, Token: ${token} - Rate limit exceeded, Attempts: ${attempts}`
        );
        res
          .status(429)
          .json({
      ...
      console.log(
        `[Password Reset Success] Email: ${email}, Token: ${token}, Resent email to: ${user.email}, Reset Link: ${resetLink}`
      );
      ...
      console.error(
        `[Password Reset Error] Email: ${email}, Token: ${token} - ${
          error instanceof Error ? error.message : "Unknown error"
        }`
      );
      res
