...
router.post('/',authenticate, RegistrationController.createRegistration);
router.get('/', RegistrationController.getAllRegistrations);
router.get('/:userId',authenticate, isAdmin, RegistrationController.getUserTicketCostSummary);
router.put('/:id', RegistrationController.updateRegistration);

// --- Ticket Management Routes ---

// Get ticket details with calculated price
router.get("/:id/details",authenticate , (req: Request, res: Response) => TicketService.getTicketDetails(req as any, res));

// Transfer a ticket to another user
router.put("/:id/transfer",authenticate, TicketService.transferTicket.bind(TicketService));

// Add tickets to a registration (increment quantity)
router.put("/:id/add-tickets",authenticate, TicketService.AddToCartTheNoOfTickets.bind(TicketService));

// Cancel specific tickets within a registration
router.put("/:id/cancel",authenticate, TicketService.cancelRegistration.bind(TicketService));

// Resend the ticket email for a registration
router.post("/:id/resend-ticket",authenticate, async (req: Request, res: Response, next: NextFunction) => { // Added NextFunction type
    try {
        ...
        const loggedInUserRoles = typedReq.user?.role ? [typeof typedReq.user.role === 'string' ? typedReq.user.role : typedReq.user.role.roleName] : [];
...
router.get("/:id/ticket-pdf",authenticate , async (req: Request, res: Response) => {
    // Use dRequest type if available, otherwise keep (req as any)
    const typedReq = req as Request;
    const loggedInUserId = typedReq.user?.userId;
    const loggedInUserRoles = typedReq.user?.role ? [typeof typedReq.user.role === 'string' ? typedReq.user.role : typedReq.user.role.roleName] : [];
