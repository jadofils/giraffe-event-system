import {
  Entity, PrimaryGeneratedColumn, Column, ManyToOne, OneToMany, JoinColumn, ManyToMany, JoinTable,
  CreateDateColumn, UpdateDateColumn, DeleteDateColumn,
} from 'typeorm';
import { IsUUID, IsNotEmpty, Length, IsEmail, IsString, IsOptional, IsPhoneNumber, IsBoolean, IsDateString, IsObject, IsUrl } from 'class-validator';
...
export class User {
  ...
  @Length(3, 50, { message: 'username must be between $constraint1 and $constraint2 characters' })
  username!: string;

  @Column()
  @IsNotEmpty({ message: 'firstName is required' })
  @Length(1, 50, { message: 'firstName must be between $constraint1 and $constraint2 characters' })
  firstName!: string;

  @Column()
  @IsNotEmpty({ message: 'lastName is required' })
  @Length(1, 50, { message: 'lastName must be between $constraint1 and $constraint2 characters' })
  lastName!: string;
  ...
  @Length(0, 50, { message: 'preferredLanguage must be at most $constraint2 characters' })
  preferredLanguage?: string;

  @Column({ type: 'varchar', length: 50, nullable: true })
  @IsOptional()
  @IsString({ message: 'timezone must be a string' })
  @Length(0, 50, { message: 'timezone must be at most $constraint2 characters' })
  timezone?: string;
  ...
  @Length(0, 100, { message: 'addressLine1 must be at most $constraint2 characters' })
  addressLine1?: string;

  @Column({ type: 'varchar', length: 100, nullable: true })
  @IsOptional()
  @IsString({ message: 'addressLine2 must be a string' })
  @Length(0, 100, { message: 'addressLine2 must be at most $constraint2 characters' })
  addressLine2?: string;
