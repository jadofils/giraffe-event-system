...
export class RegistrationService {
    ...
    private static ensureRepositoriesInitialized(): void {
        ...
        if (!RegistrationService.registrationRepository) {
            RegistrationService.registrationRepository = AppDataSource.getRepository(Registration);
            ...
            RegistrationService.ticketTypeRepository = AppDataSource.getRepository(TicketType);
     ...
     */
    static async createRegistration(data: RegistrationRequestInterface): Promise<RegistrationResponseInterface> {
        ...
        try {
            ...
            const [event, user, buyer, ticketType, venue] = await Promise.all([
                ...
                this.userRepository.findOne({ where: { userId: data.buyerId as string } }),
                this.ticketTypeRepository.findOne({ where: { ticketTypeId: data.ticketTypeId } }),
            ...
            const newRegistration = this.registrationRepository.create({
                ...
                boughtForIds: data.boughtForIds && data.boughtForIds.length > 0 ? data.boughtForIds : [],
                totalCost: totalCost,
            ...
            const fullyPopulatedRegistration = await this.registrationRepository.findOne({
                where: { registrationId: savedRegistration.registrationId },
                relations: ['event', 'user', 'buyer', 'ticketType', 'venue', 'payment', 'invoice']
            });
            ...
            const response: RegistrationResponseInterface = {
                ...
                user: {
                    ...
                    deletedAt: fullyPopulatedRegistration.user.deletedAt?.toISOString() || undefined,
                } as UserInterface,
                buyer: {
                    ...
                    deletedAt: fullyPopulatedRegistration.buyer.deletedAt?.toISOString() || undefined,
                } as UserInterface,
                ...
                venue: {
                    ...
                    deletedAt: fullyPopulatedRegistration.venue.deletedAt?.toISOString() || undefined,
                } as VenueInterface,
                ...
                paymentStatus: fullyPopulatedRegistration.paymentStatus as PaymentStatus,
                qrCode: fullyPopulatedRegistration.qrCode || undefined,
                checkDate: fullyPopulatedRegistration.checkDate ? fullyPopulatedRegistration.checkDate.toISOString() : undefined,
                ...
                payment: fullyPopulatedRegistration.payment ? {
                    ...
                    paymentDate: (fullyPopulatedRegistration.payment ?? {}) instanceof Date
                    ...
                    createdAt: fullyPopulatedRegistration.payment.createdAt?.toISOString(),
                    updatedAt: fullyPopulatedRegistration.payment.updatedAt?.toISOString(),
                    deletedAt: fullyPopulatedRegistration.payment.deletedAt?.toISOString() || undefined,
                    invoice: undefined,
                } as unknown as PaymentInterface : undefined,
                invoice: fullyPopulatedRegistration.invoice ? {
                    ...
                    invoiceDate: fullyPopulatedRegistration.invoice.invoiceDate.toString(),
                    dueDate: fullyPopulatedRegistration.invoice.dueDate.toString(),
                    totalAmount: fullyPopulatedRegistration.invoice.totalAmount,
                    status: fullyPopulatedRegistration.invoice.status,
                    createdAt: fullyPopulatedRegistration.invoice.createdAt?.toISOString(),
                    updatedAt: fullyPopulatedRegistration.invoice.updatedAt?.toISOString(),
                    deletedAt: fullyPopulatedRegistration.invoice.deletedAt?.toISOString() || undefined,
                } as InvoiceInterface : undefined,
                ...
                deletedAt: fullyPopulatedRegistration.deletedAt ? fullyPopulatedRegistration.deletedAt.toISOString() : undefined,
            };
    ...
    static async validateRegistrationIds(
        ...
        if (noOfTickets === undefined || typeof noOfTickets !== 'number' || noOfTickets <= 0) {
        ...
        if (errors.length > 0) {
            return { valid: false, message: `Validation failed: ${errors.join(" ")}`, errors: errors };
        }
        ...
        if (!user) errors.push(`User (account owner) with ID '${userId}' does not exist.`);
        if (!actualBuyerUser) errors.push(`Buyer with ID '${buyerId}' does not exist.`);
        if (!ticketType) errors.push(`Ticket Type with ID '${ticketTypeId}' does not exist.`);
    ...
    static async validateEventCapacity(
        ...
        try {
            ...
            if (!venue) {
                return { valid: false, message: `Venue with ID '${venueId}' does not exist.` };
            }

            const event = await this.eventRepository.findOne({ where: { eventId } });
            if (!event) {
                return { valid: false, message: `Event with ID '${eventId}' does not exist.` };
            }
            ...
            const totalTicketsSoldResult = await this.registrationRepository
                ...
                .andWhere("registration.paymentStatus = :status", { status: PaymentStatus.COMPLETED })
                .getRawOne();
            ...
            if (requestedTickets > remainingCapacity) {
                return { valid: false, message: `Not enough capacity. Only ${remainingCapacity} tickets left for event '${event.eventTitle}'.` };
            }

            return { valid: true };
        } catch (error) {
            console.error("Error validating event capacity:", error);
            return { valid: false, message: "An error occurred while checking event capacity." };
        }
    }
    ...
    static async validateDuplicateRegistration(
        ...
        try {
            ...
            return { valid: false, message: "An error occurred while checking for duplicate registrations." };
        }
    }
    ...
    static async validateAndCalculateTicketCost(ticketTypeId: string, quantity: number): Promise<{
        ...
        try {
            ...
            if (!ticketType) {
                return { valid: false, message: `Ticket Type with ID '${ticketTypeId}' does not exist.` };
            }
            ...
            return { valid: false, message: "An error occurred while calculating ticket cost." };
        }
    }
}