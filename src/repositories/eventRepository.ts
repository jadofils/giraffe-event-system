...
static async create(
    ...
    try {
      ...
      const userInOrg = await userRepo
        ...
        .getOne();
      if (!userInOrg) {
        return { success: false, message: 'Organizer does not belong to the specified organization.' };
      }
      ...
      if (data.venues && Array.isArray(data.venues) && data.venues.length > 0) {
        ...
        if (!(data.venues && Array.isArray(data.venues) && data.venues.every((v) => typeof v === 'string' && UUID_REGEX.test(v)))) {
          await queryRunner.rollbackTransaction();
          return { success: false, message: 'Venues must be an array of valid UUID strings.' };
        }
        ...
        if (!venueOrganizationId || !UUID_REGEX.test(venueOrganizationId)) {
          await queryRunner.rollbackTransaction();
          return { success: false, message: 'Valid venueOrganizationId is required when assigning venues.' };
        }
