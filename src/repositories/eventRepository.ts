  ...
  static async create(
    ...
    try {
      ...
      if (!userInOrg) {
        return {
          success: false,
          message: "Organizer is not part of the specified organization",
        };
      }
      ...
      if (data.venues && Array.isArray(data.venues) && data.venues.length > 0) {
        ...
        if (!data.venues.every((v) => typeof v === "string")) {
          ...
          return {
            success: false,
            message: "Venues must be an array of UUID strings",
          };
        }
        ...
        if (!venueOrganizationId) {
          ...
          return {
            success: false,
            message: "venueOrganizationId is required when assigning venues.",
          };
        }
        ...
        if (
          venueIds.length === 0 ||
          !venueIds.every((id) => UUID_REGEX.test(id))
        ) {
          await queryRunner.rollbackTransaction();
          return {
            success: false,
            message: "Invalid or empty venue IDs provided",
          };
        }

        // Check for duplicate venue IDs
        const duplicateVenueIds = venueIds.filter(
          (id, idx) => venueIds.indexOf(id) !== idx
        );
        if (duplicateVenueIds.length > 0) {
          await queryRunner.rollbackTransaction();
          return {
            success: false,
            message: `Duplicate venue IDs detected: ${[
              ...new Set(duplicateVenueIds),
            ].join(", ")}`,
          };
        }
        ...
        if (foundVenues.length !== venueIds.length) {
          ...
          return {
            success: false,
            message:
              "All selected venues must belong to the specified organization.",
          };
        }
