...
export class EventRepository {
    ...
    try {
      ...
      if (
        ...
        return {
          success: false,
          message:
            "Missing required fields: eventTitle, eventType, organizerId, startDate, endDate",
        };
      }
      ...
      if (!mappedEventType) {
        return {
          success: false,
          message: `Invalid event type. Must be one of: ${Object.values(
            EventType
          ).join(", ")}`,
        };
      }
      ...
      const userInOrg = await userRepo
        ...
        .where(
          "user.userId = :userId AND org.organizationId = :organizationId",
          {
      ...
      if (data.venues && Array.isArray(data.venues) && data.venues.length > 0) {
        ...
        if (
          !(
            ...
            data.venues.every(
              (v) => typeof v === "string" && UUID_REGEX.test(v)
            )
          )
        ) {
        ...
        if (!venueOrganizationId || !UUID_REGEX.test(venueOrganizationId)) {
          await queryRunner.rollbackTransaction();
          return {
            success: false,
            message:
              "Valid venueOrganizationId is required when assigning venues.",
          };
        }

        venueIds = [...new Set(data.venues.map((v: string) => v.trim()))]; // Remove duplicates
        ...
        const approvedVenues = foundVenues.filter(
          (v) => v.status && v.status.toLowerCase() === "approved"
        );

        if (approvedVenues.length !== venueIds.length) {
          await queryRunner.rollbackTransaction();
          const notApprovedIds = venueIds.filter(
            (id) => !approvedVenues.some((v) => v.venueId === id)
          );
          return {
            success: false,
      ...
      if (venueBookings.length > 0) {
        await queryRunner.manager.save(VenueBooking, venueBookings);
      }
      ...
      return {
        success: false,
        message: `Failed to create event and associate venues: ${
  ...
  static async getAll(): Promise<{
    ...
    try {
      const events = await CacheService.getOrSetMultiple(
        ...
        AppDataSource.getRepository(Event),
        async () =>
          await AppDataSource.getRepository(Event).find({
            relations: [
              "organizer",
              "organizer.role",
              "venues",
    ...
    try {
      ...
      if (data.venues && Array.isArray(data.venues)) {
        const venueIds = data.venues
          .map((v: VenueInterface) => v.venueId)
          .filter(Boolean);
        ...
        const venues =
          venueIds.length > 0
            ? await AppDataSource.getRepository(Venue).find({
                where: {
                  venueId: In(venueIds),
                  organization: {
                    organizationId: data.organizationId ?? event.organizationId,
        ...
        if (venueIds.length > 0 && venues.length !== venueIds.length) {
          return {
            success: false,
            message:
              "One or more venues not found or not part of the organization",
          };
        }
        event.venues = venues;
      }
      ...
      if (data.venueBookings && Array.isArray(data.venueBookings)) {
        const bookingIds = data.venueBookings
          .map((b: VenueBookingInterface) => b.bookingId)
          .filter(Boolean);
        ...
        const bookings =
          bookingIds.length > 0
            ? await AppDataSource.getRepository(VenueBooking).find({
  ...
  static async removeVenues(
    ...
    try {
      ...
      event.venues = event.venues.filter(
        (venue) => !venueIds.includes(venue.venueId)
      );
      await repo.save(event);
      await CacheService.invalidateMultiple([
        `${this.CACHE_PREFIX}${eventId}`,
  ...
  static async assignVenueBookings(
    eventId: string,
    bookingIds: string[]
  ): Promise<{ success: boolean; message?: string }> {
    if (
    ...
    try {
      ...
      if (bookings.length !== bookingIds.length) {
        return {
          success: false,
          message:
            "One or more venue bookings not found or not part of the organization",
        };
      }
  ...
  static async removeVenueBookings(
    eventId: string,
    bookingIds: string[]
  ): Promise<{ success: boolean; message?: string }> {
    if (
    ...
    try {
      ...
      event.venueBookings = event.venueBookings.filter(
        (booking) => !bookingIds.includes(booking.bookingId)
      );
      await repo.save(event);
      await CacheService.invalidateMultiple([
        `${this.CACHE_PREFIX}${eventId}`,
  ...
  static async bulkCreateVenueBookings(
    ...
    try {
      ...
      const validVenueIds = requestedVenueIds.filter((id) =>
        orgVenueIds.includes(id)
      );
      ...
      if (conflictingVenues.length > 0) {
        return {
          success: false,
          message: `Venues with IDs [${conflictingVenues.join(
            ", "
          )}] are already booked for the requested time.`,
        };
      }
      ...
      await CacheService.invalidateMultiple([
        ...
        ...validBookings.map(
          (booking) => `${this.CACHE_PREFIX}booking:${booking.bookingId}`
        ),
      ]);
  ...
  static async getVenueBookingById(
    bookingId: string
  ): Promise<{ success: boolean; data?: VenueBooking; message?: string }> {
    if (!bookingId || !UUID_REGEX.test(bookingId)) {
    ...
    try {
      const booking = await CacheService.getOrSetSingle(
        ...
        async () =>
          await AppDataSource.getRepository(VenueBooking).findOne({
            where: { bookingId },
            relations: [
              "venue",
              "user",
  ...
  static async updateVenueBooking(
    bookingId: string,
    data: Partial<VenueBookingInterface>
  ): Promise<{ success: boolean; data?: VenueBooking; message?: string }> {
    if (!bookingId || !UUID_REGEX.test(bookingId)) {
      return { success: false, message: "Valid booking ID is required" };
    }
