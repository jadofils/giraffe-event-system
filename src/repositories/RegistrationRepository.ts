...
export class RegistrationRepository {
  ...
  static async update(registrationId: string, updateData: Partial<RegistrationRequestInterface>): Promise<Registration | null> {
    try {
      ...
      if (updateData.totalCost !== undefined) registration.totalCost = updateData.totalCost;
      if (updateData.registrationStatus) registration.registrationStatus = updateData.registrationStatus;
      if (updateData.paymentId !== undefined) registration.paymentId = updateData.paymentId;
      if (updateData.invoiceId !== undefined) registration.invoiceId = updateData.invoiceId;
  ...
  static async findByQRCode(rawQrCodeDataString: string): Promise<Registration | null> {
    try {
