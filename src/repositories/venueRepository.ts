...
export class VenueRepository {
  ...
  static async update(id: string, data: Partial<VenueInterface>): Promise<{ success: boolean; data?: Venue; message?: string }> {
    ...
    try {
      const repo = AppDataSource.getRepository(Venue);
      const venue = await repo.findOne({ where: { venueId: id, deletedAt: IsNull() } });
      ...
      if ((data.venueName && data.venueName !== venue.venueName) || (data.location && data.location !== venue.location)) {
        const existingVenue = await repo.findOne({
          where: { venueName: data.venueName ?? venue.venueName, location: data.location ?? venue.location },
        });
  ...
  static async updateVenueManager(venueId: string, managerId: string): Promise<{ success: boolean; data?: Venue; message?: string }> {
    ...
    try {
      ...
      const venue = await venueRepo.findOne({ where: { venueId, deletedAt: IsNull() }, relations: ["manager"] });
      if (!venue) {
        return { success: false, message: "Venue not found or deleted." };
      }
  ...
  static async restore(id: string): Promise<{ success: boolean; data?: Venue; message?: string }> {
    if (!id) {
      return { success: false, message: "Venue ID is required." };
    }
    ...
    try {
      const repo = AppDataSource.getRepository(Venue);
      const venue = await repo.findOne({ where: { venueId: id }, withDeleted: true });
  ...
  static async getDeleted(): Promise<{ success: boolean; data?: Venue[]; message?: string }> {
  ...
  static async getVenueCount(): Promise<{ success: boolean; count?: number; message?: string }> {
    try {
      const count = await AppDataSource.getRepository(Venue).count({ where: { deletedAt: IsNull() } });
  ...
  static async removeVenueManager(venueId: string): Promise<{ success: boolean; data?: Venue; message?: string }> {
    if (!venueId) {
      return { success: false, message: "Venue ID is required." };
    }
    ...
    try {
      const repo = AppDataSource.getRepository(Venue);
      const venue = await repo.findOne({ where: { venueId, deletedAt: IsNull() } });
      if (!venue) {
        return { success: false, message: "Venue not found or deleted." };
      }
  ...
  static async getBookingsByVenue(venueId: string): Promise<{ success: boolean; data?: VenueBooking[]; message?: string }> {
    if (!venueId) {
      return { success: false, message: "Venue ID is required." };
    }
